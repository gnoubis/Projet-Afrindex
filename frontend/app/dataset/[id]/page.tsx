        ...
        <div className="flex items-center justify-between gap-3 mb-4">
          ...
          {avgRating && (
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-dm font-bold text-ink">{avgRating}</span>
            </div>
          )}
        </div>
        ...
        {sent ? (
          <div className="flex items-center gap-3 text-green-700 bg-green-50 border border-green-200 rounded-xl px-4 py-3">
            ...
            <div>
              ...
              <p className="font-dm text-xs text-green-600/70 mt-0.5">Il est maintenant visible par la communauté.</p>
            </div>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <p className="font-dm text-xs font-semibold text-earth-800/50 uppercase tracking-wide mb-2">Votre note *</p>
              <div className="flex flex-wrap items-center gap-1">
                ...
                {rating > 0 && (
                  <span className="font-dm text-sm font-medium text-earth-800/60 sm:ml-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="flex flex-col gap-1.5">
                <label className="font-dm text-xs font-semibold text-earth-800/50 uppercase tracking-wide">Votre nom</label>
                <input
                  type="text"
                  placeholder="Anonyme"
                  value={author}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="font-dm text-xs font-semibold text-earth-800/50 uppercase tracking-wide">Commentaire</label>
                <input
                  type="text"
      ...
      <div className="bg-white rounded-2xl border border-earth-200 shadow-sm p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-dm font-bold text-ink flex items-center gap-2">
            ...
            {reviews.length > 0 && (
              <span className="font-dm text-sm font-normal text-earth-800/40">({reviews.length})</span>
            )}
          </h3>
          {avgRating && (
            <div className="flex items-center gap-2 text-sm">
              <StarDisplay rating={Math.round(avgRating)} />
              <span className="font-dm font-bold text-ink">{avgRating}</span>
              <span className="font-dm text-earth-800/40 text-xs">/ 5</span>
            </div>
          )}
        </div>
        ...
        {loadingReviews ? (
              ...
              <div key={r.id} className="rounded-xl border border-earth-100 bg-earth-50/50 p-4">
                <div className="flex items-center justify-between gap-3 mb-1.5">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <span className="font-dm text-xs text-earth-800/40">
                      {new Date(r.created_at).toLocaleDateString("fr-FR")}
                    </span>
                  </div>
                </div>
                {r.comment && (
                  <p className="font-dm text-sm text-earth-800/65 leading-relaxed pl-9">{r.comment}</p>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="text-center py-8 text-earth-800/40">
            ...
            <p className="font-dm text-sm">Aucun avis pour l'instant — soyez le premier !</p>
          </div>
        )}
      </div>
    </section>
...
export default function DatasetPage() {
  ...
  if (isError || !data) {
    return (
      <div className="max-w-4xl mx-auto px-6 py-16 text-center">
        ...
        <p className="font-dm text-earth-800/60 font-medium">Dataset introuvable.</p>
        <Link href="/" className="font-dm mt-4 inline-block text-sm text-terra-500 hover:text-terra-700 underline">
          Retour à l'accueil
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
      <Link
        href="/search"
        className="inline-flex items-center gap-2 text-sm text-earth-800/50 hover:text-terra-500 mb-6 transition-colors font-dm font-medium"
      >
      ...
      {data.similar_datasets?.length > 0 && (
        <section className="mt-8">
          <div className="section-label mb-4">
            ...
            <span className="section-label-text">Aller plus loin</span>
          </div>
          <h2
            className="font-display mb-6 select-none text-ash-800"
            style={{ fontSize: "clamp(28px,3.5vw,48px)", lineHeight: 0.9, letterSpacing: "3px" }}
          >
            DATASETS<br />
